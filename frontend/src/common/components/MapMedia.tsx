...
class MapMedia extends React.Component<MapMediaProps, MapMediaState> {
  ...
  getPostcodeData() {
    let address = (this.props.postcode + " " + this.props.country).replace(
      /[, ]/g,
      "+"
    );
    let nominatimUrl =
  ...
  usingDefaults(): boolean {
    return (
      this.state.lat === this.DEFAULT_LAT &&
      this.state.long === this.DEFAULT_LONG
    );
  }
  ...
  render() {
    return (
      <div>
        ...
        {this.usingDefaults() && (
          <div className="error">
            Uh-oh! We encountered an error trying to display the map for this
            shelter. Displaying default location instead.
          </div>
        )}
      </div>
    );
