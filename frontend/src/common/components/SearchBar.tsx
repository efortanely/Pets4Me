...
class SearchBar extends React.Component<SearchBarProps, SearchBarState> {
  ...
  handleSubmit = (event: React.FormEvent<HTMLFormElement>): void => {
    ...
    this.props.history.push(
      `${this.optionsToRoute(selectValue)}?search=${encodeURIComponent(
        inputValue
      )}`
    );
        ...
        <Form.Group>
          <Form.Row>
            <Col xs={6}>
              ...
              >
                {SearchBar.options.map((option, i) => (
                  <option className="select-option" key={`${option}-${i}`}>
                    {option}
                  </option>
                ))}
              </Form.Control>
            ...
            <Col xs={6}>
              ...
              <Form.Control
                ...
                size="lg"
                type="text"
                placeholder={`Search ${this.state.searchOption}...`}
              />
            </Col>
          </Form.Row>
        </Form.Group>
