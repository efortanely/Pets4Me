...
import {
  Pets4meSheltersService,
  Pets4meDogBreedsService,
  Pets4meCatBreedsService,
} from "../../common/services/Pets4meModelInstanceService";
...
class ShelterInstancePage extends React.Component<ShelterProps, ShelterState> {
  static providers: ShelterProviders = {
    ...
    catBreedsService: Pets4meCatBreedsService,
  };
  ...
  fetchShelter = (shelter_id: string): Promise<Shelter> => {
    const pets4meShelterService: ModelInstanceService<Shelter> =
      ShelterInstancePage.providers.sheltersService;
    return pets4meShelterService.getInstanceById(shelter_id);
  };

  fetchDogBreed = (dog_breed_id: string): Promise<DogBreed> => {
    const pets4meDogBreedService: ModelInstanceService<DogBreed> =
      ShelterInstancePage.providers.dogBreedsService;
    return pets4meDogBreedService.getInstanceById(dog_breed_id);
  };

  fetchCatBreed = (cat_breed_id: string): Promise<CatBreed> => {
    const pets4meCatBreedService: ModelInstanceService<CatBreed> =
      ShelterInstancePage.providers.catBreedsService;
          ...
          <Row>
            ...
            <Col className="shelter-phone">
              <Row>
                ...
                />
                <p id="phone-number">
                  {this.genericEmpty(shelter.contact?.phone_number)}
                </p>
              </Row>
            </Col>
          </Row>
  ...
  getDisplayShelterAddress = (shelter: Shelter) => {
    let address =
      this.formatData(shelter.address?.address1, " ") +
      this.formatData(shelter.address?.address2, ", ") +
      this.formatData(shelter.address?.city, ", ") +
      this.formatData(shelter.address?.state, ", ") +
      this.formatData(shelter.address?.postcode, " ");
    if (address.length === 0) {
      ...
      <Container>
        <Row>
          <Col>
            ...
            <Row>
              <p className="top-breed-id">
                {this.getLinkedUrl(
                  shelter.top_dog_breed_id,
                  "dog-breeds",
                  this.genericEmpty(dogBreed.name)
                )}
              </p>
            </Row>
          </Col>
          <Col>
            ...
            <Row>
              <p className="top-breed-id">
                {this.getLinkedUrl(
                  shelter.top_cat_breed_id,
                  "cat-breeds",
                  this.genericEmpty(catBreed.name)
                )}
              </p>
            </Row>
          </Col>
  ...
  getContact(): string {
    if (
      this.state.shelter?.contact?.email ||
      this.state.shelter?.contact?.phone_number
    )
      return `${this.formatData(
        this.state.shelter.contact?.email,
        ", "
      )}${this.formatData(this.state.shelter.contact?.phone_number, "")}`;
      ...
      <div className="model-instancepage">
        <MediaQuery className="mobile" query="(max-width: 1349px)">
          ...
          <div className="mapMedia">
            <MapMedia
              address={this.getApiShelterAddress(shelter)}
          ...
          <div className="adoption-policy">
            ...
            <p id="adoption-policy">
              {this.genericEmpty(shelter.adoption_policy)}
            </p>
          </div>
          ...
          {all_pets_ids.length === 0 ? (
            <p className="cards">No pets available</p>
          ) : (
