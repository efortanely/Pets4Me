...
interface DogBreedProps extends Partial<RouteComponentProps> {
  breed: DogBreed;
}
interface DogBreedState {
  breed: DogBreed;
}
interface DogBreedProviders {
  dogBreedsService: ModelInstanceService<DogBreed>;
}

class DogBreedInstancePage extends React.Component<
  DogBreedProps,
  DogBreedState
> {
  ...
  fetchDogBreed = (breed_id: string): Promise<DogBreed> => {
    const pets4meDogBreedService: ModelInstanceService<DogBreed> =
      DogBreedInstancePage.providers.dogBreedsService;
  ...
  getAttributes = (
    height_imperial: { low: React.ReactNode; high: React.ReactNode },
    weight_imperial: { low: React.ReactNode; high: React.ReactNode },
    life_span: { low: React.ReactNode; high: React.ReactNode },
    bred_for: string
  ): JSX.Element => {
    return (
      <Container>
        <Row className="attributes">
          <Col className="height">
            <p>Height</p>
            <h4 id="height">
              {height_imperial?.low} - {height_imperial?.high} in.
            </h4>
          </Col>
          <Col className="weight">
            <p>Weight</p>
            <h4 id="weight">
              {weight_imperial?.low} - {weight_imperial?.high} lb.
            </h4>
          </Col>
          <Col className="life-span">
            <p>Life Span</p>
            <h4 id="life-span">
              {life_span?.low} - {life_span?.high} yr.
            </h4>
          </Col>
  ...
  componentDidMount() {
    ...
    if (this.getCurrentBreedId() !== breed_id) {
      this.fetchDogBreed(breed_id).then(this.updateDogBreed).catch(console.log);
    }
  }
  ...
  genericEmpty(value: string): string {
    if (isNullOrUndefined(value) || value.length === 0) return "Unknown";
    return value;
  }
      ...
      <div className="model-instancepage">
        <MediaQuery className="mobile" query="(max-width: 1299px)">
          <div className="instancepage-header">
            ...
            <p id="group">
              {this.genericEmpty(breed.breed_group)
                ? this.genericEmpty(breed.breed_group)
                : ""}
            </p>
          </div>
          ...
          {this.getAttributes(
            breed.height_imperial,
            breed.weight_imperial,
            breed.life_span,
            breed.bred_for
          )}
          ...
          {this.state.breed.dog_ids?.length === 0 ? (
            <p className="cards">No dogs available</p>
          ) : (
            <PetsInfoCarousel itemIds={breed.dog_ids} />
          )}
          <br />
          <h2 className="cards-headers">Shelters with {breed.name}s</h2>
          {this.state.breed.shelters_with_breed?.length === 0 ? (
            <p className="cards">No shelters found</p>
          ) : (
