...
interface CatBreedProviders {
  catBreedService: ModelInstanceService<CatBreed>;
}

class CatBreedInstancePage extends React.Component<
  CatBreedProps,
  CatBreedState
> {
  static providers: CatBreedProviders = {
  ...
  fetchCatBreed = (breed_id: string): Promise<CatBreed> => {
    const pets4meCatBreedService: ModelInstanceService<CatBreed> =
      CatBreedInstancePage.providers.catBreedService;
      ...
      <Container>
        <Row className="attributes">
          ...
          <Col className="child-friendly">
            <p>Child Friendly</p>
            <Row>{this.getStars(child_friendly)}</Row>
          </Col>
          <Col className="dog-friendly">
            <p>Dog Friendly</p>
            <Row>{this.getStars(dog_friendly)}</Row>
          </Col>
          <Col className="grooming-level">
            <p>Grooming</p>
            <Row>{this.getStars(grooming_level)}</Row>
          </Col>
        </Row>
      </Container>
    );
  ...
  componentDidMount() {
    ...
    if (this.getCurrentBreedId() !== breed_id) {
      this.fetchCatBreed(breed_id).then(this.updateCatBreed).catch(console.log);
    }
  }
      ...
      <div className="model-instancepage">
        <MediaQuery className="mobile" query="(max-width: 1299px)">
          ...
          <div className="alternate-names">
            ...
            <p id="alt-names">
              {!breed.alt_names || breed.alt_names.length === 0
                ? "No alternate names specified"
                : breed.alt_names.join(", ")}
            </p>
          </div>
          ...
          {this.state.breed.cat_ids?.length === 0 ? (
            <p className="cards">No cats available</p>
          ) : (
