...
interface PetInstancePageProviders {
...
class PetInstancePage extends React.Component<PetProps, PetState> {
  static providers: PetInstancePageProviders = {
  ...
  getBreedHeader = (
    ...
    let primary_breed_link = this.getLinkedUrl(
      primary_breed,
      this.getBreedType(),
      "Breed"
    );
    let secondary_breed_link = this.getLinkedUrl(
      secondary_breed,
      this.getBreedType(),
      "Breed"
    );
  ...
  getShelterButton = (
    backendEntity: BackendEntity,
    type: string = "shelters"
  ): JSX.Element => {
    if (backendEntity?.id) {
      let route = `/${type}/${backendEntity.id}`;
      return (
        <Button className="instancepage-button" href={route}>
          More About {backendEntity.name}
        </Button>
      );
    }
    return <div></div>;
    ...
    return (
      <div className="model-instancepage">
        <MediaQuery className="mobile" query="(max-width: 1349px)">
          ...
          <Button className="instancepage-button" href={pet.url}>
            More About {pet.name}
          </Button>

          <div className="shelter-name">
            <h4 className="shelter-name-for-testing">
              {this.genericEmpty(pet.shelter?.name)}
            </h4>
        ...
        <MediaQuery className="desktop" query="(min-width: 1350px)">
          <Col>
            ...
            <Row className="buttons">
              ...
              <Col md={{ offset: 1 }}>{this.getShelterButton(pet.shelter)}</Col>
            </Row>
          </Col>
        </MediaQuery>
      </div>
