...
export const constructQuery = (selectedFilters: PetsFiltersState) => {
  ...
  if (selectedFilters.species) {
    ...
    if (selectedFilters.primaryBreed.length > 0) {
      ...
      if (selectedFilters.species === "Dog")
        filters.push({
          name: "primary_dog_breed",
          op: "has",
          val: { name: "name", op: "in", val: selectedFilters.primaryBreed },
        });
      else if (selectedFilters.species === "Cat")
        filters.push({
          name: "primary_cat_breed",
          op: "has",
          val: { name: "name", op: "in", val: selectedFilters.primaryBreed },
        });
    }
    if (selectedFilters.secondaryBreed.length > 0) {
      ...
      if (selectedFilters.species === "Dog")
        filters.push({
          name: "secondary_dog_breed",
          op: "has",
          val: { name: "name", op: "in", val: selectedFilters.secondaryBreed },
        });
      else if (selectedFilters.species === "Cat")
        filters.push({
          name: "secondary_cat_breed",
          op: "has",
          val: { name: "name", op: "in", val: selectedFilters.secondaryBreed },
        });
    }
  ...
  if (selectedFilters.sortType && selectedFilters.sortType.length > 0) {
    if (
      selectedFilters.sortType === "primary_dog_breed" ||
      selectedFilters.sortType === "secondary_dog_breed" ||
      selectedFilters.sortType === "size" ||
      selectedFilters.sortType === "age"
    )
...
export class PetsFilters extends React.Component<
  PetsFilterOptions,
  PetsFiltersState
> {
  ...
  render() {
    return (
      <div className="filters">
        ...
        <Select
          isMulti
          isDisabled={!this.state.species}
          options={this.getBreedData()}
          placeholder={this.breedPlaceholderText("primary")}
          isClearable={true}
        ...
        <Select
          ...
          isDisabled={!this.state.species}
          options={this.getBreedData()}
          placeholder={this.breedPlaceholderText("secondary")}
          isClearable={true}
          onChange={(newFilters: any) => {
            if (newFilters) {
              this.setState({
