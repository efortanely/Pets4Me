...
export const constructQuery = (breeds: string[], selectedFilters: DogBreedsFiltersState) => {
    let filters = []
    let order_by = []
    let query = ""
    ...
    if (selectedFilters.breedGroup.length > 0)
        filters.push({ "name": "breed_group",       "op": "in", "val": selectedFilters.breedGroup})
    filters.push({ "name": "height_imperial_low",   "op": "ge", "val": selectedFilters.minHeight})
    filters.push({ "name": "height_imperial_high",  "op": "le", "val": selectedFilters.maxHeight})
    filters.push({ "name": "weight_imperial_low",   "op": "ge", "val": selectedFilters.minWeight})
    filters.push({ "name": "weight_imperial_high",  "op": "le", "val": selectedFilters.maxWeight})
    filters.push({ "name": "life_span_low",         "op": "ge", "val": selectedFilters.lifespanMin})
    filters.push({ "name": "life_span_high",        "op": "le", "val": selectedFilters.lifespanMax})
