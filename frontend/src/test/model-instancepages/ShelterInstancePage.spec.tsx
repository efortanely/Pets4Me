...
describe("<ShelterInstancePage/>", () => {
  ...
  function spyOnSheltersService(shelter: Shelter) {
    let testSheltersService: ModelInstanceService<Shelter> = mockModelInstanceService<
      Shelter
    >(shelter);
  ...
  beforeEach(() => {
    ...
    testShelter = {
      ...
      photos: { full: [], small: [] },
      all_pets: {
        "0": { name: "bob", species: "Cat", id: 1 },
        "1": { name: "Alice", species: "Dog", id: 2 },
      },
      distance: 1,
      adoption_policy: "bar",
      mission: "mar",
  ...
  it("should not crash when Shelter is undefined", () => {
    ...
    let testPage = mount(
      <ShelterInstancePage
        shelter={undefined}
        match={{
          params: { shelter_id: `${testShelter.id}` },
          isExact: true,
          path: "",
          url: "",
        }}
  ...
  it("should not crash when api Shelter is empty", () => {
    ...
    let testPage: ReactWrapper<ShelterInstancePage> = mountWithShelter(
      emptyShelter
    );
  ...
  it("should render all details on mobile", () => {
    ...
    expect(mobile_elements.address().text())
      .to.include(testShelter.address.address1)
    ...
    expect(mobile_elements.email().text()).to.include(
      testShelter.contact.email
    );
    expect(mobile_elements.phone_number().text()).to.include(
      testShelter.contact.phone_number
    );
    expect(mobile_elements.adoption_policy().text()).to.include(
      testShelter.adoption_policy
    );
  ...
  it("should render all details on desktop", () => {
    ...
    expect(desktop_elements.address().text())
      .to.include(testShelter.address.address1)
      .and.to.include(testShelter.address.address2)
      .and.to.include(testShelter.address.postcode)
      .and.to.include(testShelter.address.city)
      .and.to.include(testShelter.address.state);
    expect(desktop_elements.email().text()).to.include(
      testShelter.contact.email
    );
    expect(desktop_elements.phone_number().text()).to.include(
      testShelter.contact.phone_number
    );
    expect(desktop_elements.adoption_policy().text()).to.include(
      testShelter.adoption_policy
    );
  ...
  it("should GET shelter on component mount if no shelter supplied in props", () => {
    ...
    expect(getShelterSpy.getInstanceById).to.have.been.calledWith(
      `${testShelter.id}`
    );
  });
});
