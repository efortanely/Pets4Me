...
interface HomeState { numPets: number }
interface HomeProviders { petsService: ModelInstanceService<Pet> }
...
class Home extends React.Component<any, HomeState> {
  ...
  getTotalNumPets = () => {
      this.getModelInstanceService().getModelPageOfInstances(1, {resultsPerPage: 1})
      .then(this.updateNumPets)
  }

  getRandomPetLink(): JSX.Element {
    return <Link to={`/pets/${Math.floor(Math.random() * this.state.numPets) + 1}`}><h2 id='adopt'>adopt a pet today</h2></Link>
  }
