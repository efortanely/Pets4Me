...
export class Member extends React.Component<MemberProps, MemberState> {
  ...
  componentDidMount() {
    ...
    gitlabApiService.getCommits().then((res) => this.countCommits(res));
  ...
  countCommits(contributors: Commit[]) {
    let count: number = contributors.filter((value) =>
      this.props.author_name.some((name) => name === value.author_name)
    ).length;
