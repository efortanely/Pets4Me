...
from sqlalchemy import Column, Integer, String, Boolean, ForeignKey, Float, alias, or_, and_
...
class Pet(db.Model):
    ...
    primary_dog_breed_name = Column(String(100))
    secondary_dog_breed_name = Column(String(100))
    primary_cat_breed_name = Column(String(100))
    secondary_cat_breed_name = Column(String(100))
    ...
    def query(cls):
        ...
        for pet in db.session.query(Pet):
            ...
            if pet.primary_dog_breed:
                pet.primary_dog_breed_name = pet.primary_dog_breed.name
            if pet.secondary_dog_breed:
                pet.secondary_dog_breed_name = pet.secondary_dog_breed.name
            if pet.primary_cat_breed:
                pet.primary_cat_breed_name = pet.primary_cat_breed.name
            if pet.secondary_cat_breed:
                pet.secondary_cat_breed_name = pet.secondary_cat_breed.name
...
class Shelter(db.Model):
    ...
    dist = Column(Float)
    has_cats = Column(Boolean)
    has_dogs = Column(Boolean)
    ...
    def query(cls):
        ...
        for shelter in db.session.query(Shelter):
            ...
            dogs = db.session.query(Shelter.id)                           \
            .join(Pet, Shelter.pets)                                      \
            .filter(and_(Shelter.id == shelter.id, Pet.species == "Dog")) \
            .all()

            shelter.has_dogs = len(dogs) > 0

            cats = db.session.query(Shelter.id)                           \
            .join(Pet, Shelter.pets)                                      \
            .filter(and_(Shelter.id == shelter.id, Pet.species == "Cat")) \
            .all()

            shelter.has_cats = len(cats) > 0
